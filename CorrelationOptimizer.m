...
%%
[a, X, ~,~,~, trialidx] = selectData(experiment, todo_fish, stims2use, trialn2usein,'trialavg');
ntrials = numel(X);
...
for i_trial = 1:numel(todo_trials)
    ...
    figure; 
    for i=1:niter+1
        thiscorrmat = 1-squareform(pdist(atmpMAT(:,:,i), 'correlation'));
        imagesc(thiscorrmat)
        caxis([0 1])
...
figure; imagesc(zscore(y(idx,:)')'); caxis([-4 4])
yticks(1:ntrials); yticklabels(stims(Xsort))

figure
thiscorrmat = 1-squareform(pdist(y(Xsort,:), 'correlation'));
imagesc(thiscorrmat)
yticks(1:ntrials); yticklabels(stims(Xsort))
