...
%     tmp = selectCells(tmp,data.L/ds,alltopunits');
    ...
    %zscore
%     T = size(tmp,1);
%     tmp = zscore(traceFormat(permute(tmp,[1,3,2])));
...
%     interval = [floor(nanmax([1, (data.stim_off_sec + 60)*fs/ds])) : ...
...
l = legend('baseline','odor'); legend('boxoff')
l.TextColor = 'w';
...
idx = data.idx_by_stim_type(1:33)-1;
subplot(122); scatter(t,y(idx,:),'w','filled'); hold on
