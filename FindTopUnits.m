...
thistraces = traceFormat(data.tracesdn,data.L);
cellstd = squeeze(std(thistraces(interval,:,:),[],1));
cellvar = cellstd.^2;
...
xticks(1:34); xticklabels(data.stim_type(data.idx_by_stim_type)); xtickangle(90)
yticks(1:34); yticklabels(data.stim_type(data.idx_by_stim_type)); xtickangle(90)
title('portion of common top variant units')
%------------
end
