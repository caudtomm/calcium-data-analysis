function data = selectAnatRegions(data, override)
%%% REGION SELECTION

% 
% clear all
% disp('start'); 
% load('W:\scratch\gfriedri\caudtomm\ev_data\TC_200717_TC0004_01_sxpDp_DATA.mat')

% startup
% cd(paths.localdata)
load(fullfile('defROIs',data.trials{1}))
anatomy = plane{1}.anatomy;
clear plane
...
a = data.ROI_map_common; a(a>0) = quantile(anatomy(:), .1);
...
if override || ~isfield(data,'anat_regions')
    i_region = 1;
    isInROI = [];
    [h, cells_in_region, region_names] = deal({});
else
    h = data.anat_regions.position;
    cells_in_region = data.anat_regions.cells(1:end-1);
    region_names = data.anat_regions.names(1:end-1);
...
data.anat_regions.position = h;
data.anat_regions.names = region_names;
data.anat_regions.cells = cells_in_region;
